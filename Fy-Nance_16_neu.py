...
def neu_zeichnen(idx_von, idx_bis):
    ...
    # ----------------------------------------------------------------------     
    ax.clear()   
    ax.set_title(f"[16neu]Ticker:{akt_Num}/{akt_Nam}/{akt_Tik} von {idx_von} bis {idx_bis}",fontsize=12, fontweight='bold')
    ...
    if not df_zeitfenster.empty:
        ...
        ax.set_title(f"[16neu]Ticker:{akt_Num}/{akt_Nam}/{akt_Tik} {str_start} + {tage_differenz} Tage = {str_end}")
        ...
        if len(diff_data) > 5:
            ...
            if N > 5:
                 ...
                 # ----------------------------------------------------------------------
                 # 2. Top 3 "Resonanzen" (Formanten) des Boersen-Ereignisses finden
            ...
            if FFT_Diagram:           
               ax3.set_xscale('log')
               ax3.set_title(f"[16neu]/{akt_Nam}/{akt_Tik} Log-skaliertes dMA-Spektrum", fontsize=10, fontweight='bold')
