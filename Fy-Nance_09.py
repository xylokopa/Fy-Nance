# Fy-Nance_09.py 03-06-2026 Projekt Ticker-Oszillograph
import pandas as pd
...
try:
    ...
    # Automatische On-Board-Ticker-Liste Datei beim Testen fehlt
...
plt.subplots_adjust(left=0.22, bottom=0.32, right=0.82)
...
ax_von = plt.axes([0.22, 0.05, 0.22, 0.03])
ax_online = plt.axes([0.47, 0.04, 0.06, 0.05])
ax_bis = plt.axes([0.56, 0.05, 0.22, 0.03])

ax_box_start = plt.axes([0.22, 0.14, 0.12, 0.04])
ax_box_end = plt.axes([0.66, 0.14, 0.12, 0.04])
text_mitte = fig.text(0.36, 0.14, '', bbox=dict(facecolor='white', alpha=1.0, edgecolor='black'))

ax_diag1 = plt.axes([0.84, 0.23, 0.13, 0.04])
ax_diag2 = plt.axes([0.84, 0.17, 0.13, 0.04])
ax_diag3 = plt.axes([0.84, 0.11, 0.13, 0.04])
ax_diag4 = plt.axes([0.84, 0.05, 0.13, 0.04])
...
btn_online = Button(ax_online, 'ON' if online_schalter_val else 'OFF', color='green' if online_schalter_val else 'red')
...
# CLR-Schalter direkt unter dem Online-Schalter
ax_clr = plt.axes([0.47, 0.005, 0.06, 0.035]) # unter ax_online [0.47, 0.04, 0.06, 0.05]

# Variable für den CLR-Zustand (1 = FIX / Zoom behalten, 0 = RESET / Vollansicht)
clr_schalter_val = 1 

# Button-Objekt initialisieren
btn_clr = Button(ax_clr, 'FIX', color='green')
...
def on_online_clicked(event):
    ...
    if online_schalter_val > 0:
        ...
        btn_online.label.set_text("OFF")
        btn_online.color = "red"
    else:
        online_schalter_val = 1
        btn_online.label.set_text("ON")
