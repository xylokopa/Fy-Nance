# Fy-Nance_15_neu.py 15-06-2026 17h37 723 Zeilen Ticker-Oszillograph R.Wu_GastH_Nr178854
...
def neu_zeichnen(idx_von, idx_bis):
    ...
    ax.clear()   
    ax.set_title(f"[15neu]Ticker:{akt_Num}/{akt_Nam}/{akt_Tik} von {idx_von} bis {idx_bis}",fontsize=12, fontweight='bold')
    ...
    txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|292')
    ...
    if not df_zeitfenster.empty:
        ...
        ax.set_title(f"[15neu]Ticker:{akt_Num}/{akt_Nam}/{akt_Tik} {str_start} + {tage_differenz} Tage = {str_end}")
        ...
        # 4. Hauptlinien zeichnen (Nutzt noch das ganze df, Matplotlib clippt das per xlim)
        if preis_linie:
        ...
        # ----------------------------------------------------------------------
        diff_dropna = df_zeitfenster['Diff'].dropna() # <-- REDUZIERT auf df_zeitfenster
        ...
        if len(diff_data) > 5:
            ...
            if FFT_Diagram:           
               ax3.set_xscale('log')
               ax3.set_title(f"[15neu]/{akt_Nam}/{akt_Tik} Log-skaliertes dMA-Spektrum", fontsize=10, fontweight='bold')
...
################################
        ...
        if not df.empty:
            ...
            txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|480')
            ...
            if val_clr == 1 and idx_bis_alt <= max_index_neu:
                ...
                txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|496')
                ...
                txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|516')
...
def on_diag2_clicked(event):
    ...
    FFT_Diagram = 0    # aus  
    df_zeitfenster, str_start, str_end, idx_von, idx_bis = von_bis_ablesen()
    # txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|629')
...
def on_diag3_clicked(event):
    ...
    FFT_Diagram = 0    # aus
    df_zeitfenster, str_start, str_end, idx_von, idx_bis = von_bis_ablesen()
    # txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|650')
    status = ' Status : idx_von = '+str(idx_von)+' bis '+' idx_bis = '+str(idx_bis)
    txt_box_status.set_val(status) 
    neu_zeichnen(idx_von,idx_bis)
    plt.show()
...
def on_diag4_clicked(event):
    ...
    df_zeitfenster, str_start, str_end, idx_von, idx_bis = von_bis_ablesen()
    # txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|671')
    status = ' Status : idx_von = '+str(idx_von)+' bis '+' idx_bis = '+str(idx_bis)
    txt_box_status.set_val(status) 
    neu_zeichnen(idx_von,idx_bis)
    plt.show()
...
def on_submit_start(text_input):
    try:
        ...
        txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|683')
...
# ---------------------------------------------------------------------------------
txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|712')
...
txt_box_status.set_val(txt_box_status.text_disp.get_text()+'|716')
